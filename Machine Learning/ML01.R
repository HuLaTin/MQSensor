...
SensorData <- SensorData[1:200,2:22]
...
# eventTemp[,1:ncol(eventTemp)] <- as.data.frame(lapply(eventTemp[,1:ncol(eventTemp)], normalize))
events <- as.data.frame(t(events))

# write.csv(events, file = "events.csv")
#write.csv(SensorData, file = "20191213Rafael.csv")
