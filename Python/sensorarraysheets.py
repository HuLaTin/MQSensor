...
while True:
    ...
    #close file being written
        ...
        creds = ServiceAccountCredentials.from_json_keyfile_name('arraydata.json', scope)
        client = gspread.authorize(creds)
        sheet = client.open("ArrayData").sheet1

        now = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S.%f')
        print(now)
    #data = 1
        values =[now, mq2sensorValue, LPGppm, mq4sensorValue, CH4ppm, mq5sensorValue,
                 MQ5LPGppm, mq6sensorValue, MQ6LPGppm, mq7sensorValue, H2ppm,
                 mq8sensorValue, MQ8H2ppm, mq9sensorValue, COppm, mq135sensorValue,
                 Uppm, bme680.temperature, bme680.gas, bme680.humidity,bme680.pressure,
                 bme680.altitude,res['apm10'], res['apm25'], res['apm100'],
                 res['pm10'], res['apm25'], res['pm100'], res['gt03um'], res['gt05um'],
                 res['gt10um'],res['gt25um'], res['gt50um'], res['gt100um']]
